...
class UserUpdateForm(forms.Form):
    ...
    language = forms.ModelChoiceField(
        ...
        widget=forms.Select(attrs={'class': 'form-select'}), label='Язык программирования')
    send_email = forms.BooleanField(
        required=False, widget=forms.CheckboxInput, label='Получать рассылку?')
...
class ContactForm(forms.Form):
    city = forms.CharField(
        required=True, widget=forms.TextInput(attrs={'class': 'form-select'}),
        label='Город'
    )
